...
from .wws_info import get_AccountInfo,new_get_AccountInfo
from .wws_recent import get_RecentInfo,new_get_RecentInfo
...
WWS_help ="""
    ...
    wws 绑定 服务器 游戏昵称
    wws 查询绑定/wws 查绑定
    wws [服务器+游戏昵称][@群友][me]：查询账号总体战绩，参数顺序随意
    wws [服务器+游戏昵称][@群友][me] recent [日期]：查询账号近期战绩，默认1天，参数顺序随意
...
async def selet_command(bot,ev:CQEvent):
    try:
        ...
        if not select_command:
            msg = await new_get_AccountInfo(qqid,search_list)
            ...
            select_command,search_list = await find_and_replace_keywords(search_list,command_list)             #第二次匹配
            if not select_command:
                msg = await new_get_RecentInfo(qqid,search_list)
            elif select_command == 'ship':
                msg = '待开发：查单船近期战绩'
            else:
                msg = '：看不懂指令QAQ'
